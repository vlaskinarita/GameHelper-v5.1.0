...
internal class SafeMemoryHandle : SafeHandleZeroOrMinusOneIsInvalid
{
	...
	internal int ReadStdMap<TKey, TValue>(StdMap nativeContainer, int maxSizeAllowed, bool enableCounting, Func<TKey, TValue, bool> onEachNotNullNode) where TKey : unmanaged where TValue : unmanaged
	{
		...
		Queue<StdMapNode<TKey, TValue>> first64Childrens = new Queue<StdMapNode<TKey, TValue>>(64);
		int totalChildrenProcessed = processSubTree(first64Childrens, parent, 64);
		Parallel.ForEach(first64Childrens, new ParallelOptions
		{
			MaxDegreeOfParallelism = Core.GHSettings.EntityReaderMaxDegreeOfParallelism
		}, () => (new Queue<StdMapNode<TKey, TValue>>(2000), 0), delegate(StdMapNode<TKey, TValue> first32Child, ParallelLoopState _, long _, (Queue<StdMapNode<TKey, TValue>>, int) localState)
		{
	...
	internal TValue[] ReadStdBucket<TValue>(StdBucket nativeContainer) where TValue : unmanaged
	{
		if (nativeContainer.Data.First == IntPtr.Zero || nativeContainer.Capacity <= 0)
		{
			return Array.Empty<TValue>();
		}
		return ReadStdVector<TValue>(nativeContainer.Data);
	}
